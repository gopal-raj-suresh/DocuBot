...
class LogManager:
    ...
    async def add_log(self, log: AgentLog) -> None:
        ...
        with self._lock:
            ...
            # Get subscriber queues (copy to avoid modification during iteration)
            subscribers = list(self._subscribers[log.job_id])

        # Notify all subscribers for this job (outside lock to allow async operations)
        dead_queues = []
        ...
        if dead_queues:
            with self._lock:
                for queue in dead_queues:
                    if queue in self._subscribers[log.job_id]:
    ...
    async def subscribe(self, job_id: str) -> asyncio.Queue:
        ...
        with self._lock:
            ...
            existing_logs = list(self._logs.get(job_id, []))

        # Send existing logs immediately (outside lock to allow async operations)
