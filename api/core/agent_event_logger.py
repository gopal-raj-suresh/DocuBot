...
class AgentEventLogger(AsyncCallbackHandler):
    ...
    def __init__(self, job_id: str, agent_name: str):
        ...
        """
        # Don't call super().__init__() to avoid creating locks in wrong event loop
        # super().__init__()
        self.job_id = job_id
...
def create_dual_callbacks(job_id: str, agent_name: str, model_name: str) -> list:
    """
    Factory function to create callback handlers for agents

    NOTE: Langfuse integration now uses manual trace creation instead of callbacks.
    Use create_langfuse_trace() before agent execution and log_agent_generation() after.

    Args:
        job_id: Job ID for log streaming
        agent_name: Name of the agent (CodeExplorer, Writer, etc.)
        model_name: Model name being used (for Langfuse metadata - currently unused)

    Returns:
        List of callback handlers (AgentEventLogger for UI streaming)

    Example:
        callbacks = create_dual_callbacks("job_123", "CodeExplorer", "Qwen/Qwen3-4B-Instruct-2507")
        result = await agent.ainvoke(..., config={"callbacks": callbacks})
    """
    # Only UI logger - Langfuse is now handled via manual trace creation
    return [AgentEventLogger(job_id=job_id, agent_name=agent_name)]
