...
class SimplifiedDocuBotWorkflow:
    """
    ...
    async def assembly_node(self, state: DocGenState) -> DocGenState:
        """Assemble final README with the template structure"""
        job_id = state["job_id"]
        ...
        readme_parts = []
        readme_sections = state.get("readme_sections") or {}
        mermaid_diagrams = state.get("mermaid_diagrams") or {}
        ...
        if readme_sections and "Project Overview" in readme_sections:
...
_workflow: SimplifiedDocuBotWorkflow = None


async def get_workflow() -> SimplifiedDocuBotWorkflow:
    """Get or create workflow instance"""
    global _workflow
    if _workflow is None:
        _workflow = SimplifiedDocuBotWorkflow()
